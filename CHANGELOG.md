...
- The filesystem mounts a read-only tree at a directory (`vfs.mount(path,
  ...
  with `EROFS`, and a snapshot leaves it out. The embedder answers the tree.
- A process's identity carries what it was started as, where it started and
...
- A guest that saves `Error.prepareStackTrace` while one of Node's stack
  overrides is pending and restores it later restores its own function; it
  restored the engine's, and every later stack in the realm recursed.
- An override whose stack is never read lapses at the end of its task instead
  of holding its error and keeping every stack on the fallback formatter.

## v0.5.3 — 2026-09-23

- A virtual WebSocket's handshake carries `Origin` and `Host` as a browser
  sends them, when the page's side names them on `connect`: the page's origin,
  and the authority the client addressed rather than the engine's
  `localhost:<port>`. A guest server that checks a socket's origin against its
  host (Workbench's terminal does) refused every socket from the tab.
