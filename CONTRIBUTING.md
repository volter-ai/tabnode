...
tabnode is Node's own library in a browser tab, and a change to it goes toward Node. This page is the short form; [`FORK.md`](FORK.md) has the whole of it, and [`CONSTITUTION.md`](CONSTITUTION.md) is what the engine must remain.
...
- **It is measured.** A module's number comes from Node's own suite run against a build (`scripts/node-tests.mjs`, [`scripts/NODE-TESTS.md`](scripts/NODE-TESTS.md)); a change that moves a number says so, and updates the module's row in `BUILTINS.md` and `FORK.md`.

## How to send one

1. Fork, branch from `main`, `npm install`, `npm run build:lib`.
2. Make the change, with `npm run type-check` clean.
3. Open a pull request against `main` with the commit message as the description. There is no CI to wait on; a maintainer reads the diff and merges it.

Releases are cut by a maintainer from `main` with `scripts/release-volter.sh`, which builds, tags `v0.2.14-volter.<n>` on the `release` branch and publishes `@volter/tabnode` at that version.
